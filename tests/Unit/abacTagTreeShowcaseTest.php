 ...
 *       новинка недели (6)
 */
class abacTagTreeShowcaseTest extends TestCase
{
    ...
    /**
     * "Акция" and everything below it, when the tag itself should count too.
     */
    public function test_the_tag_itself_and_everything_below(): void
    {
        Config::set('access.functions', ['tagsUnder' => TagsUnder::class]);
        $this->app->scoped(TagsUnder::class);

        $this->user->addPermission('products.view', when: "exists(product.tags, name == 'акция' || id in tagsUnder('акция')) && ".self::LIKED);
