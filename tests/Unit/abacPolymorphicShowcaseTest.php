 ...
 */
class abacPolymorphicShowcaseTest extends TestCase
{
    /** @var TestUser */
    protected $user;
    ...
    public function test_a_condition_on_a_model_missing_from_config_is_refused(): void
    {
        Config::set('access.resources', ['product' => Product::class, 'comment' => Comment::class, 'tag' => Tag::class]);

        $this->expectExceptionMessage('Like reached by "likes.count" is not listed in config access.resources');

        $this->user->addPermission('products.view', when: 'exists(product.comments, likes.count > 10)');
    }
