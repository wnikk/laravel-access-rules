 ...
 */
class PerformancePromisesTest extends FeatureTestCase
{
    ...
    /**
     * config access.hierarchy: one WITH RECURSIVE query, or a query per level as version 2 did.
     * Whichever is configured, a user behind a diamond of roles three levels deep gets the same
     * permissions. "auto" is what projects run with, so it is asked too.
     *
     *     user -> editor -> staff -> everyone
     *          -> author -> staff
     *     "outsider" is inherited by nobody
     */
    #[DataProvider('waysToCollectInheritance')]
    public function test_every_way_of_collecting_inheritance_gives_the_same_permissions(string $way): void
    {
        Config::set('access.hierarchy', $way);
        Config::set('access.owner_types', [TestUser::class, 'Role']);

        foreach (['deep.view', 'staff.view', 'taken.away', 'other.view'] as $rule) {
            $this->getAccessRules()->newRule($rule, $rule);
        }
        foreach (['everyone', 'staff', 'editor', 'author', 'outsider'] as $role) {
            Access::for('Role', $role)->create($role);
        }

        Access::for('Role', 'staff')->inheritFrom('Role', 'everyone');
        Access::for('Role', 'editor')->inheritFrom('Role', 'staff');
        Access::for('Role', 'author')->inheritFrom('Role', 'staff');

        Access::for('Role', 'everyone')->allow('deep.view');
        Access::for('Role', 'staff')->allow('staff.view');
        Access::for('Role', 'editor')->allow('taken.away');
        Access::for('Role', 'everyone')->deny('taken.away');
        Access::for('Role', 'outsider')->allow('other.view');

        $user = TestUser::factory()->make()->forceFill(['id' => 7]);
        $user->inheritPermissionFrom('Role', 'editor');
        $user->inheritPermissionFrom('Role', 'author');

        $this->assertTrue($user->hasPermission('deep.view'), 'three levels up');
        $this->assertTrue($user->hasPermission('staff.view'), 'reached by two paths, counted once');
        $this->assertFalse($user->hasPermission('taken.away'), 'an inherited prohibition beats an inherited permission, however far it is');
        $this->assertNull($user->hasPermission('other.view'), 'a role nobody inherits from gives nothing');
        $this->assertSame(['deep.view', 'staff.view'], collect($this->getAccessRules()->getAllPermittedRule($user))->sort()->values()->all());
    }

    public static function waysToCollectInheritance(): array
    {
        return ['one recursive query' => ['cte'], 'a query per level, as version 2' => ['loop'], 'chosen by the package' => ['auto']];
    }
