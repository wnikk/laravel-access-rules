...
namespace Wnikk\LaravelAccessRules\Conditions\Syntax;
 ...
 * ['and', a, b], ['or', a, b], ['cmp', op, l, r], ['in', l, r, negated].
 */
final class Parser
{
     ...
     * comparisons and "in" 4. Matching SQL matters, because the same tree becomes a WHERE clause
     * and a reader compares the two.
     */
    private const PREC = ['||' => 1, 'or' => 1, '&&' => 2, 'and' => 2,
        '=='                   => 4, '!=' => 4, '>' => 4, '>=' => 4, '<' => 4, '<=' => 4, 'in' => 4, 'not' => 4];
    ...
    private function expr(int $minPrec): array
    {
        ...
        while (($t = $this->peek()) && $t[0] !== 'val' && isset(self::PREC[$t[1]])) {
            $op = $t[1];
            if ($op === 'not' && ($this->peek(1)[1] ?? null) !== 'in') {
                break;
            }
            $prec = self::PREC[$op];
            if ($prec < $minPrec) {
