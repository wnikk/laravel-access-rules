 ...
 * refusal to remember the name of the ability, and in debug mode the provider hands it every
 * check that Gate has refused. The rest of the
...
final class Explainer
{
    ...
    /**
     * Debug mode: every refusal is explained and every filtered list is written down, for log().
     *
     * The intended use is a support session. A user complains, an administrator signs in as that
     * user and ticks "debug": a middleware of the application calls Access::debug(), and the error
    ...
    /**
     * Called for every check Gate has finished, while somebody in this process watches.
     * Only a final refusal is explained: a check that the package left open and a policy then
     * permitted is no refusal, and explaining it would fill the log with false alarms.
     *
     * @param array{0:int, 1:string|int|null}|null $owner Whom the package checked the user as, from GateHook::owner().
     */
    public function evaluated(GateEvaluated $event, ?array $owner): void
    {
        if (! $this->enabled()) {
            return;
        }

        $refused = $event->result instanceof Response ? $event->result->denied() : ! $event->result;

        if ($refused) {
            $this->denied($owner[0] ?? null, $owner[1] ?? null, $event->user instanceof Model ? $event->user : null, $event->ability, $event->arguments, true);
        }
    }
     ...
     */
    public function denied(?int $type, string|int|null $id, ?Model $subject, string $ability, array $args, bool $byGate = false): string
    {
        if ($type === null) {
        ...
        $report['by_gate'] = $byGate;
        $report['text']    = $this->describe($report);
        $this->denials[]   = $report;
    ...
    public function describe(array $report): string
    {
        ...
        if (($report['by_gate'] ?? false) && $report['decision'] === true) {
            $lines[] = 'The package permits, and Laravel Gate still refused: another Gate::before callback answered first.';
        } elseif (($report['by_gate'] ?? false) && $report['decision'] === false) {
            $lines[] = 'A prohibition takes the permission away and is no veto: a policy or Gate::define() for this ability could still permit, and none did.';
        }
